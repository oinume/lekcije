...
// CreateCustomToken is the resolver for the createCustomToken field.
func (r *mutationResolver) CreateCustomToken(ctx context.Context) (string, error) {
	user, err := authenticateFromContext(ctx, r.userUsecase)
	if err != nil {
		return "", err
	}
	customToken, err := r.userUsecase.CreateCustomToken(ctx, user)
	if err != nil {
		return "", err
	}
