...
func (m *crawlerMain) run(args []string) error {
	...
	var (
		concurrency     = flag.Int("concurrency", 1, "Concurrency of crawler. (default=1)")
		continueOnError = flag.Bool("continue", true, "Continue to crawl if any error occurred. (default=true)")
		specifiedIDs    = flag.String("ids", "", "Teacher IDs")
		followedOnly    = flag.Bool("followedOnly", false, "Crawl followedOnly teachers")
		all             = flag.Bool("all", false, "Crawl all teachers ordered by evaluation")
		newOnly         = flag.Bool("new", false, "Crawl all teachers ordered by new")
		interval        = flag.Duration("interval", 1*time.Second, "Fetch interval. (default=1s)")
